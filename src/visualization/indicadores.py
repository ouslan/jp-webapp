...
def web_app_indicadores(request):
    api_url = creds[6]
    df_monthly = (
        pd.read_parquet("data/processed/indicadores_economicos.parquet")
        .sort_values(by="date")
        .reset_index()
    )
    df_monthly = requests.get(f"{api_url}/indicadores_economicos").json()
    df_quarterly = (
        pd.read_parquet("data/processed/indicadores_trimestrales.parquet")
                        ...
                        args2=[
                            {
                                ...
                                "y": [df_quarterly[y_axis_options[0]]],
                            }
                        ],
                    ),
                    dict(
