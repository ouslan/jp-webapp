...
def web_app_imports_exports(request):
    df1_imports = requests.get(
        f"{API_URL}/data/trade/jp/?agg=yearly&types=country"
    ).json()
    ...
    if request.method == "POST":
        imports_params = {
            ...
            "second_dropdown": request.POST.get(
                "second_dropdown", frequency_defaults["second_dropdown"]
            ),
            "third_dropdown": request.POST.get(
                "third_dropdown", frequency_defaults["third_dropdown"]
            ),
        }
    else:
        imports_params = frequency_defaults

    imports_time_start, imports_time_end = get_time_range(
        imports_params["frequency"],
        imports_params["second_dropdown"],
        imports_params["third_dropdown"],
    )
    ...
    imports_chart = generate_pie_chart(
        imports_data,
        "imports",
        "country_name",
        f"{imports_params['frequency']} / {imports_params['second_dropdown']} / {imports_params['third_dropdown']}",
    )
    ...
    if request.method == "POST":
        ...
        exports_params = {
            ...
            if frequency_2
            else frequency_defaults["frequency"],
            "second_dropdown": second_dropdown_2
            if second_dropdown_2
            else frequency_defaults["second_dropdown"],
            "third_dropdown": third_dropdown_2
            if third_dropdown_2
            else frequency_defaults["third_dropdown"],
        }
    else:
        exports_params = frequency_defaults

    exports_time_start, exports_time_end = get_time_range(
        exports_params["frequency"],
        exports_params["second_dropdown"],
        exports_params["third_dropdown"],
    )
    ...
    exports_chart = generate_pie_chart(
        exports_data,
        "exports",
        "country_name",
        f"{exports_params['frequency']} / {exports_params['second_dropdown']}",
    )
