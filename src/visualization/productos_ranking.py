...
def productos_ranking(request):
    ...
    response = requests.get("https://api.econlabs.net/data/trade/moving?agr=false").json()
    ...
    df_last_month_imports = df[(df['date'] == last_month) & (df["moving_import_rank"] != 0)]
    df_last_month_exports = df[(df['date'] == last_month) & (df["moving_export_rank"] != 0)]
    ...
    fig_export_top.add_trace(go.Bar(
    ...
    fig_export_bottom.add_trace(go.Bar(
    ...
    fig_import_top.add_trace(go.Bar(
    ...
    fig_import_bottom.add_trace(go.Bar(
